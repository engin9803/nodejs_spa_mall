...
router.get("/goods/cart", async (req, res) => {
  ...
  // cart schemas에서 가져오는 코드
  const goodsIds = carts.map((cart) => cart.goodsId);
  // goods schemas에서 가져오는 코드
...
router.post("/goods", async (req, res) => {
  const { goodsId, name, thumbnailUrl, category, price } = req.body;
    // Goods.find에서 find는 프로미스를 반환 하기 때문에 await을 사용 await을 사용하려면 async가 필수
  ...
  if (goods.length) {
    return res.status(400).json({ success: false, errorMessage: "이미 있는 데이터 입니다." });
                                  ...
                                  // create - insert역할 까지 해주며 await을 안할 시 입력이 끝나기 전에 응답을 해버림
